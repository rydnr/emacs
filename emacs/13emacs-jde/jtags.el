...
;;                         Johan Dykström (johan.dykstrom@home.se)
;; Created:                May 2001
;; Version:                0.92
;; Keywords:               java, tags, etags

;; Copyright (C) 2002 Alexander Baltatzis.
;;
...
;;
;; - jtags-find-tag: looks up and shows the definition of the method at point
;; - jtags-member-completion: finds all completions of the partly typed method 
;;                            name at point
;;
;; Installation:
;;
...
;;
;; Windows: dir /b /s *.java | etags --members --language=java -
;;
;; UNIX:    find `pwd` | egrep java$ | etags --members --language=java -
;;
;; Include the tags table files in your tags-table-list in init file, 
...
;; 
;; 
;; Functions that operate on the tags file:
;; 
;; 
;; Functions that parses Java files:
;; 
;; 
;; HTML functions:
;; 
...
(defvar running-xemacs (string-match "XEmacs\\|Lucid" emacs-version)
  "Variable to distinguish which emacs is running. Value t if Xemacs,
and nil if GNU Emacs.")

(defvar jtags-file
  (if (and (buffer-file-name)
		   (string-match "jtags.el" (buffer-file-name)))
	  (buffer-file-name)
	load-file-name)
...
(defvar jtags-last-possible-completions nil
  "Holds a list of the text to be replaced and possible completions. This 
variable is used to circle completions")

(defvar jtags-trace-on nil
  "*Set to true to get my-trace-message to actually print things.")

(defvar jtags-trace-to-tmp nil
  "Set to true to make jtags-message send trace messages to the buffer
\"*tmp\" in addition to the bottom of the screen and the
\"*Messages*\" buffer. This is useful when a lot of messages are
printed in a short time.")

(defun jtags-message (&rest args)
  (when jtags-trace-on
	(apply 'message args)
	(when jtags-trace-to-tmp
	  (save-excursion
...
\(global-set-key \[\(meta ,\)\] 'jtags-find-tag\)"
  ...
  (let* ((orig-buffer (buffer-name))
	...
	(message "Tag not found!")
      ...
      (message "Found %s in %s at line %d" (cadddr definition) (car definition) (cadr definition))
      ;; Check doc path
...
\(global-set-key \[\(ctrl ,\)\] 'jtags-member-completion\)
...
This method could be replaced and integrated to dabbrev-expand instead."
  ...
  (let (orig_pos p)
	...
	(jtags-message "in jtags-member-completion, jtags-find-member-completions returned: %S"
				   jtags-last-possible-completions)
    (if jtags-last-possible-completions
	(progn
	  
	  ;; Replace text in buffer, if there is more than one completion, or
	  ;; this is the first time we run the command
...
;;       - Handle "this" and "super" (in jtags-local-tags-lookup?)
;;       - Consider multiple methods with same name but different type.

(defun jtags-recursive-tags-lookup (identifiers classes)
  "Looks up a tag recursively. IDENTIFIERS is a list of identifiers,
starting with the first identifier in the expression, and ending with
the identifier to look up. CLASS-LIST is a list of classes to search
for the identifier. If the tag is found, the following list is
returned:

list'(filename            ;; The name of the file containing the definition
      line                ;; The line number where the definition starts
      class               ;; The class in which the identifier is defined
      identifier)         ;; The name of the identifier

If the tag is not found, nil is returned."
  ;; (jtags-message "in jtags-recursive-tags-lookup: identifiers = %S classes = %S" identifiers classes)
  (if (null identifiers)
      nil
    (let ((res nil))
	...
	(if (= (length identifiers) 1)
	    (list (car res) (caddr res) (cadr res) (car identifiers))
	  (jtags-recursive-tags-lookup (cdr identifiers) 
				    (jtags-get-class-list (cadr res))))))))  

(defun jtags-local-tags-lookup (identifiers class-list)
  "Looks up a local variable definition. IDENTIFIERS is a list of
identifiers, starting with the first identifier in the expression, and
ending with the identifier to look up. CLASS-LIST is a list of classes
to search for the identifier. If the tag is found, the following list
is returned:
...
If the tag is not found, nil is returned."
  ...
  (let* ((boundary (nth 3 (tags-lookup-method (car class-list))))
    ...
    (if local
		...
		;; filename, line, type, and name of the local variable.
		(if (cdr identifiers)
			(jtags-recursive-tags-lookup (cdr identifiers) 
										 (jtags-get-class-list (car local)))
		  (list (buffer-file-name) (cadr local) (car local) (car identifiers))))))
...
(defun jtags-find-member-completions ()
  "Looks up the identifier in the marked expression, or where the
point is positioned, and returns a list containing the text to be
completed, and the completable members of the class. If there are no
matching members in the class, nil is returned."
  (interactive)
  (save-excursion
    (let* ((identifiers (reverse (parse-java-line)))
      ...
      (unless definition
		(setq definition (jtags-local-tags-lookup identifiers classes)))
      (jtags-message "in jtags-find-member-completions: definition = %s" definition)
      (when definition
		...
		(when members
		  ...
		  ;; If there are any members left, return a list beginning with the 
		  ;; identifier to be completed, and ending with the matching members
...
list'(filename      ;; Name of the source file where class is defined
      ...
      first-method-pos ;; The file position of the first method. This can be 
                       ;; used if only part of the file needs to be opened in 
                       ;; order to search for extends, implements etc.
